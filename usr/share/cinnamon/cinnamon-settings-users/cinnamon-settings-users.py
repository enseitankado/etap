...
#USER CALLBACKS
    ...
    def on_user_addition(self, event):
        ...
        if response == Gtk.ResponseType.OK:
            ...
            if dialog.account_type_combo.get_active() == 1:
                subprocess.call(["usermod", username, "-G", "%s,sudo,cdrom,floppy,audio,dip,video,plugdev,netdev,bluetooth,scanner,lpadmin" % username])
            else:
                subprocess.call(["usermod", username, "-G", "%s,cdrom,floppy,audio,dip,video,plugdev,netdev,bluetooth,scanner,lpadmin" % username])
            self.load_groups()
        dialog.destroy()
