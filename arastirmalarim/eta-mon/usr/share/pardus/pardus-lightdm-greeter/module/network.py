...
def hostname_to_unique_ip(hostname):
    try:
        addresses = socket.getaddrinfo(hostname, None)
        unique_ips = list({addr[4][0] for addr in addresses})
        return unique_ips[0] if unique_ips else "IP adresi bulunamadı"
    except socket.gaierror as e:
        return f"Hostname çözümleme hatası: {e}"
...
def update_popover_text():
    ...
    if _last_network_label_text != network_label_text:
        ...
        ip_adresi = hostname_to_unique_ip(SUNUCU_AD)
        okul_ici_sunucu = f"\nOkul sunucu:\n - {SUNUCU_AD}:{ip_adresi}"
        _last_network_label_text += okul_ici_sunucu

        loginwindow.o("ui_label_network").set_text(_last_network_label_text)
